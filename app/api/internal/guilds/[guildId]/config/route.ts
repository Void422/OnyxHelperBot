...
import { automodRules, guildSettings, levelRoles, logConfigurations } from "@/db/schema";
...
export async function GET(request: Request, context: Context) {
  try {
    ...
    const [[settings], [logs], rules, rewards] = await Promise.all([
    ...
    return json({ settings: settings ?? null, logs: logs ?? null, automodRules: rules, levelRoles: rewards });
  } catch (error) {
