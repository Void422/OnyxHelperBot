  ...
  return <DashboardShell guildId={guildId} guildName={guild.guildName} active="automod">
    ...
    {!active ? <div className="loading-stack"><div className="skeleton" /><div className="skeleton" /></div> : <div className="automod-layout">
      ...
      <div className="rule-editor">
        ...
        <section className="settings-card"><h2>Exemptions</h2><p>Staff with Manage Messages are always exempt. Add server-specific roles and channels below.</p><div className="split-pickers"><div><div className="field-label">Roles</div><div className="picker-list">{guild.resources?.roles.slice(0,30).map((role) => <label key={role.id}><input type="checkbox" checked={active.exemptRoleIds.includes(role.id)} onChange={() => toggleValue("exemptRoleIds", role.id)} /><span>{role.name}</span></label>)}</div></div><div><div className="field-label">Channels</div><div className="picker-list">{textChannels.map((channel) => <label key={channel.id}><input type="checkbox" checked={active.exemptChannelIds.includes(channel.id)} onChange={() => toggleValue("exemptChannelIds", channel.id)} /><span>#{channel.name}</span></label>)}</div></div></div></section>
      </div>
    </div>}
