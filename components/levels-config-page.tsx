...
import { rankLadderPresets, type LevelCurve, type RankLadderPreset } from "@/packages/core/src/rank-ladders";
...
interface LevelConfig { rewards: Reward[]; xp: { curve?: LevelCurve; baseXp?: number; growthXp?: number; cooldownSeconds?: number; minimumMessageLength?: number; minAward?: number; maxAward?: number; excludedChannelIds?: string[]; excludedRoleIds?: string[] }; levelAnnouncementChannelId?: string; levelUpMessage?: MessageTemplate }

function normalized(value: LevelConfig): LevelConfig {
  return { ...value, xp: { curve: "standard", baseXp: 100, growthXp: 250, cooldownSeconds: 60, minimumMessageLength: 8, minAward: 10, maxAward: 20, excludedChannelIds: [], excludedRoleIds: [], ...value.xp }, rewards: [...value.rewards].sort((left, right) => left.level - right.level), levelUpMessage: value.levelUpMessage ?? { content: "◆ {mention} reached **level {level}** and moved up the ranks." } };
}
...
export function LevelsConfigPage({ guildId }: { guildId: string }) {
  ...
  const curveEstimate = useMemo(() => {
    if (!draft) return [];
    const averageAward = Math.max(1, ((draft.xp.minAward ?? 10) + (draft.xp.maxAward ?? 20)) / 2);
    return [0, 1, 5, 10, 25, 50].map((level) => {
      const required = xpForLevel(level + 1, draft.xp) - xpForLevel(level, draft.xp);
      return { level, required, messages: Math.ceil(required / averageAward) };
    });
  }, [draft]);
  ...
  return <DashboardShell guildId={guildId} guildName={guild.guildName} active="levels">
    ...
    {!draft ? <div className="loading-stack"><div className="skeleton" /><div className="skeleton" /></div> : <div className="configuration-grid">
      <div>
        ...
        <section className="settings-card"><div className="section-title-row"><div><div className="page-kicker">XP pace</div><h2>Make the climb count</h2><p>Choose a preset or build the exact XP curve your server should grind through.</p></div><Gauge size={22} /></div><div className="form-grid">
          <div className="field"><label htmlFor="xp-curve">Progression curve</label><select id="xp-curve" className="select" value={draft.xp.curve} onChange={(event) => updateXp({ curve: event.target.value as LevelCurve })}><option value="standard">Momentum · quick season</option><option value="grind">The Grind · long-term</option><option value="legendary">Legend · brutal</option><option value="custom">Custom · build your own</option></select><small>Level 100 takes {xpForLevel(100, draft.xp).toLocaleString()} total XP.</small></div>
          <div className="field"><label htmlFor="xp-cooldown">Cooldown (seconds)</label><input id="xp-cooldown" className="input" type="number" min={0} max={86400} value={draft.xp.cooldownSeconds} onChange={(event) => updateXp({ cooldownSeconds: Number(event.target.value) })} /><small>{draft.xp.cooldownSeconds === 0 ? "No time gate: every eligible message can earn XP." : "How often one member can earn XP. Set 0 for no time gate."}</small></div>
          <div className="field"><label htmlFor="xp-length">Minimum message length</label><input id="xp-length" className="input" type="number" min={0} max={2000} value={draft.xp.minimumMessageLength} onChange={(event) => updateXp({ minimumMessageLength: Number(event.target.value) })} /><small>Set 0 for no character requirement. Duplicate and low-signal protection still applies.</small></div>
          ...
          {draft.xp.curve === "custom" && <><div className="field"><label htmlFor="xp-base">Level 0 → 1 cost</label><input id="xp-base" className="input" type="number" min={1} max={10000000} value={draft.xp.baseXp} onChange={(event) => updateXp({ baseXp: Number(event.target.value) })} /><small>XP required for the very first level.</small></div><div className="field"><label htmlFor="xp-growth">Extra XP each level</label><input id="xp-growth" className="input" type="number" min={0} max={10000000} value={draft.xp.growthXp} onChange={(event) => updateXp({ growthXp: Number(event.target.value) })} /><small>Added to every next level cost. Set 0 for a flat curve.</small></div></>}
        </div><div className="curve-forecast"><div className="curve-forecast-head"><div><strong>Grind forecast</strong><span>Estimated eligible messages, using the midpoint of your XP award range.</span></div><span>{((draft.xp.minAward ?? 10) + (draft.xp.maxAward ?? 20)) / 2} XP avg</span></div><div className="curve-estimate-grid">{curveEstimate.map((item) => <article key={item.level}><small>Level {item.level} → {item.level + 1}</small><strong>{item.required.toLocaleString()} XP</strong><span>~{item.messages.toLocaleString()} messages</span></article>)}</div></div></section>
